...
def synthetic_dataset_1(cluster_density=0.75):
    n_samples = int(400*cluster_density)
    
    # Generate two dense clusters
    X1, _ = make_blobs(n_samples=n_samples, centers=[[0, -0.33]], cluster_std=0.1)
    X2, _ = make_blobs(n_samples=n_samples, centers=[[0, 0.33]], cluster_std=0.1)

    # Generate circular pattern
    X_circle, _ = make_circles(n_samples=n_samples*2, factor=0.99, noise=0.05)

    # Generate sparse noise around
    X_noise = np.random.uniform(-1.5, 1.5, (n_samples*3, 2))

    # Remove points inside the circumference centered at (0,0) with radius 1.1
    distance_from_center = np.sqrt(np.sum(X_noise**2, axis=1))
    X_noise = X_noise[distance_from_center > 1.2]
...
def synthetic_dataset_2(cluster_density=0.75):
    ...
    # Generate two dense clusters
    X1, _ = make_blobs(n_samples=n_samples, centers=[[-3/4*Pi+0.1, 0]], cluster_std=0.2)
    X2, _ = make_blobs(n_samples=n_samples, centers=[[-1/4*Pi-0.1, 0]], cluster_std=0.2)

    # Generate sinusoidal cluster
    X_sin_raw = np.random.uniform(-Pi, Pi, n_samples*2)
    sin_noise = np.random.normal(0, 0.05, n_samples*2)
...
def synthetic_dataset_3(cluster_density=0.75):
    n_samples = int(400*cluster_density)
    
    # Generate two dense clusters
    X1, _ = make_blobs(n_samples=n_samples, centers=[[-0.5, 0]], cluster_std=0.1)
    X2, _ = make_blobs(n_samples=n_samples, centers=[[0.5, 0]], cluster_std=0.1)

    # Generate sparse noise around
    X_noise = np.random.uniform(-1, 1, (n_samples*3, 2))
